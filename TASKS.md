...
- [ ] Route non-narrative panel tags to structured state, not the main text pane — `room objs`/`room players` are handled (see the structured-room-state item below), and `progressBar`/`indicator` now are too (see the vitals-tracking item below — `lib/grimoire/vitals_tracker.rb`). Everything else is, as of 2026-09-13, a **temporary wholesale squelch, not real routing**: `lib/grimoire/panel_tag_tracker.rb` (`PanelTagTracker`) drops `dialogData`, `openDialog`, `roommeta`, `spell`, `left`, `right`, `resource`, `style`, `skin`, `image`, `pulse`, `label`, `compass`, `castTime` outright — confirmed necessary against a real live-captured session (see `docs/decisions.md`: 38 of 54 blank lines in a real parsed-output log traced to this, plus one garbled `spell`/`left`/`right` concatenation), not just the fixtures. `inv`'s container-listing shape (`spec/fixtures/inventory.xml` "Shape C") still leaks into narrative too and is not in `PanelTagTracker`'s list. Revisit `PanelTagTracker`'s remaining drop list item-by-item once a UI actually wants any of this (prepared-spell/hands) — see `docs/decisions.md` for the exact migration path `room objs`/`room players` and now `progressBar`/`indicator` already took.
...
- [ ] Basic vitals/indicator area (health, mana, stamina, roundtime) fed by structured stream state — stretch goal, may land after the first working scrollback-and-input loop. Health/mana/stamina/spirit/mind/encumbrance/stance bars plus an active-indicators label now land in `Window#build_vitals_strip`/`#update_vitals`, fed from `App#handle_line` on every line (not gated behind `text.empty?`, since a vitals-only line carries no narrative text at all) — see `window_spec.rb`'s `#update_vitals` group and `app_spec.rb`'s "refreshes the vitals strip" case. Roundtime is still open: `PromptTracker` captures `<prompt time="...">`'s raw epoch, but nothing yet turns it into a ticking countdown (needs a real wall-clock diff plus a GLib timeout to redraw it live) — the tag that actually carries a roundtime *end* time (as opposed to the prompt's own timestamp) has not been confirmed against live traffic yet either. Bars are colorized via a GTK CSS provider (`Window#load_vitals_css`), one `vital-<field>` class per bar, fed from fixed defaults in `lib/grimoire/vitals_colors.rb` (`VitalsColors`) built on the plain `Grimoire::Color` RGB triple (`lib/grimoire/color.rb`) — `_references/uberbar_eo.lic` was checked first and has no color/RGB data of its own (it renders through Wrayth's own named skin/image assets), so the defaults instead follow the color convention common to Simutronics-client bar mods generally (confirmed with the user 2026-09-13): background/empty `0,0,0`, health red, mana blue, stamina gold, spirit white/light-gray, mind purple, encumbrance/stance neutral gray. No selection/configuration mechanism exists yet — deliberately deferred, see "Out of scope for MVP" below. Bar height was also bumped to a `Window::BAR_HEIGHT = 20` CSS `min-height` default (four times the ~5px the base GTK theme renders on its own) — user-reported as too thin to read at a glance; same "fixed default, no configuration yet" scope as the colors above.
...
- [ ] User-selectable/configurable vitals-strip colors — `VitalsColors`'s defaults (see the "Basic vitals/indicator area" item above) are fixed constants for now, with no settings file, UI, or env var to override them
