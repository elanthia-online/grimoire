...
- [ ] lich-5: emit the `begin`/`end` markers from `detachable_client_send_init`, with `end` guaranteed on every path (ok, skipped when game modules never load, error from the rescue), plus specs.
- [ ] grimoire: on connect, display nothing; on `begin`, expect an end marker; on `end status='ok'`, display a local connection status line in the game window, then send the first `look`.
- [ ] grimoire fallbacks: no `begin` within ~2s of connect means an older Lich (keep today's first-prompt `look`, plus a quiet-room timer); `begin` without `end` within ~12s, or `end` with `skipped`/`error`, shows a warning status line and still looks.
- [ ] grimoire: decide whether live game lines arriving between `begin` and `end` display immediately (current recommendation) or are held until the status line.
